...
from typing import IO, AnyStr, Generator, Optional, Literal

try:
    import torch
    from torch import Tensor
except ImportError:
    print(f"Running experiments requires a pytorch install in your environment")
    exit(1)
...
class ExperimentArgs:
    ...
    n_splits: int
    experiment_dir: Path
    architectures: list[Literal["iter", "asp", "diffusionner"]] = None
    dev: bool = False
...
def run_experiment(
    args: ExperimentArgs,
    watcher_class: "type[ExperimentWatcher]" = None
):
    ...
    if not args.summary_only:
        log_file = args.experiment_dir / (("run_experiment" if not args.dev else "dev_run_experiment") + ".log")
        if "CUDA_VISIBLE_DEVICES" in os.environ or "RANK" in os.environ:
            log_file = log_file.with_name(log_file.name + ".cuda" + os.environ.get("RANK", os.environ.get("CUDA_VISIBLE_DEVICES", "0")))
        log_file.parent.mkdir(parents=True, exist_ok=True)
        log_handlers.append(
            main_log := logging.FileHandler(log_file, mode='a')
        )
...
class ExperimentStatus(enum.Enum):
    STATUS_NOT_STARTED: int = 0
    STATUS_ALREADY_STARTED: int = 1
    STATUS_NEEDS_MORE_GPU: int = 2
    STATUS_DONE: int = 3
...
def cleanup_datasets(datasets: list[str]):
    return list(sorted({
        (dataset if "_split" not in dataset else dataset.split("_split")[0]) for dataset in datasets
    }))
...
class ExperimentWatcher:
    ...
    def get_experiment_dir(info: ExperimentInfo, base_dir: Path):
        ...
        return base_dir / info.architecture / cleanup_name(info.dataset) / cleanup_name(info.model)
    ...
    def get_experiment_state(self, info: ExperimentInfo):
        experiment_run_file = self.get_experiment_dir(info, self.status_dir) / self.get_run_name(info, False)
        experiment_done_file = self.get_experiment_dir(info, self.status_dir) / self.get_run_name(info, True)
    ...
    def missing_runs(
        ...
        for experiment in self.compute_run_info():
            ...
            if (
                experiment_state == ExperimentStatus.STATUS_NOT_STARTED
            ):
                yield experiment
    ...
    def summary(self):
        ...
        for experiment in self.compute_run_info():
            experiment_dir = self.get_experiment_dir(experiment, self.experiment_dir) / self.get_run_name(experiment)
            experiment_metrics = experiment_dir / "metrics.json"
            if not experiment_metrics.exists():
                skip_model_dataset_arch.add((experiment.architecture, experiment.model, experiment.dataset))
                continue

            experiment_key = (experiment.dataset, experiment.architecture, experiment.model)
        ...
        def compute_std_mean_best(vals: Tensor, best_run: int) -> tuple[float, float, float]:
            vals = vals.squeeze()
        ...
        for (dataset, architecture, model), experiment_metrics in metrics.items():
            if any(len(v) < num_runs for k, v in experiment_metrics.items()):
                self.logger.info(f"Skipping model '{model}' because metrics files are not available")
                continue
            experiment_metrics = OrderedDict({k: torch.tensor(v) for k, v in experiment_metrics.items()})
            experiment_bound_metrics = bound_metrics[(dataset, architecture, model)]
            experiment_bound_metrics = OrderedDict({k: torch.tensor(v) for k, v in experiment_bound_metrics.items()})
            ...
            for k, v in experiment_metrics.items():
                ...
                if k in experiment_bound_metrics:
                    bound = compute_std_mean_best(experiment_bound_metrics[k], best_performing_run)
    ...
    def run(self):
        ...
        while (
            (info := next(self.missing_runs(), None)) is not None
            and not self.stop_iteration
        ):
            successful = False
            try:
            ...
            if not successful:
                self.delete_run(info)
        if info is None:
            self.logger.info(f"Completed {len(list(self.compute_run_info()))} experiments")
        ...
        for info in self.compute_run_info():
            ...
            if state == ExperimentStatus.STATUS_ALREADY_STARTED:
                if self.cleanup_status_dir:
                    if (++removed) == 0:
                        self.logger.info(f"Performing cleanup ...")
                    run_file = self.get_experiment_dir(info, self.status_dir) / self.get_run_name(info, False)
                    self.logger.info("\t" + run_file.as_posix())
                    os.remove(run_file)
                else:
                    if (++removed) == 0:
                        self.logger.info(f"Currently running jobs:")
    ...
    def run_experiment(self, info: ExperimentInfo) -> bool:
        ...
        if experiment_done_file.exists():
            warnings.warn(f"Attempted to run completed experiment ...")
            return True
        ...
        if not self.is_rank_zero():
            while not experiment_run_file.exists():
                self.logger.info(f"Waiting for rank zero to start the training process ...")
                sleep(10)

            experiment_log_file = experiment_run_file.with_name(
                experiment_log_file.name + "_rank" + os.environ.get("RANK", "0") + ".log"
            )
        else:
            experiment_run_file.touch(exist_ok=True)
            self.logger.info(f"Created run status file {experiment_run_file.as_posix()}")

        experiment_dir = self.get_experiment_dir(info, self.experiment_dir) / self.get_run_name(info)
        ...
        try:
            with open(experiment_log_file, "w") as log_file:
                ...
                self.logger.info(f"Process {info.dataset}/{info.architecture}/{info.model}/{self.get_run_name(info)} produced return code {output.returncode}")
    ...
    def get_iter_run_args(self, info: ExperimentInfo):
        args = [
            "python3",
            (self.work_dir / "related_work" / "ITER" / "train.py").as_posix(),
            "--transformer",
            info.model,
            "--dataset",
            info.dataset if not self.use_seed_as_dataset else info.dataset + "_split" + str(info.seed),
            "--no_use_tqdm",
            "--verbose",
        ]
        if self.use_bfloat16:
            args.append("--use_bfloat16")
        if self.use_fsdp:
            args.append("--use_fsdp")
        args.append("--seed")
        args.append(str(info.seed))
        if self.load_ckpt:
            args.extend(['--load_ckpt', self.load_ckpt.as_posix()])
        args.append("--model_path")
        args.append((self.get_runtime_dir(info) / self.get_run_name(info, False)).as_posix())
        return args

    def get_diffusionner_run_args(self, info: ExperimentInfo):
        assert self.use_seed_as_dataset
        config_path = Path(info.dataset)
        config = config_path.stem
        if "_split" not in config:
            config = config + "_split" + str(info.seed)
        config = config + ".conf"
        config = config_path.parent / config
        # if not config.is_absolute():
        #    config = config.absolute()
        train_dir = (
            (self.get_runtime_dir(info) / self.get_run_name(info, False))
            .absolute()
            .as_posix()
        )
        args = [
            "python3",
            (self.work_dir / "related_work/DiffusionNER/diffusionner.py").as_posix(),
            "train",
            "--config",
            (self.work_dir / config).as_posix(),
            "--save_path",
            train_dir,
            "--seed",
            str(info.seed),
    ...
    def get_asp_run_args(self, info: ExperimentInfo):
        assert self.use_seed_as_dataset
        config_path = Path(info.dataset)
        config = config_path.stem + "_split" + str(info.seed) + ".conf"
        config = config_path.parent / config
        args = [
            "python3",
            (self.work_dir / "related_work/ASP/run_ner.py").as_posix(),
            (self.work_dir / config).as_posix(),
