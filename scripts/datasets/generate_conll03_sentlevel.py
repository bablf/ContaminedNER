        ...
        for index, (token, tag) in enumerate(zip(elem_tokens, elem_tags)):
            if tag == 'O':
                ...
                if tag_type == 'B':
                    begins += 1
                if (
                    tag_category != current_category
                    and current_category is not None
                    and current_entity
                ) or (
                    tag_type == 'B' and current_entity
                ):
                    assert current_entity
                    entities.append(
                        {
...
if __name__ == '__main__':
    ...
    create_dataset(dataset_dict["train"], output_filename="datasets/conll03s/conll03s_train.json")
    create_dataset(dataset_dict["validation"], output_filename="datasets/conll03s/conll03s_dev.json")
    create_dataset(dataset_dict["test"], output_filename="datasets/conll03s/conll03s_test.json")
